...
def eval_a_model(model_to_eval, loader, loss_func, plot=False, num_batch=-1):
    ...
    # for d_idx, data in enumerate(loader):
        ...
        for goalset in goal_pred:
            # print(f'goalset {goalset.shape}')
            for m_goalset in goalset.permute(1,0,2):
                # print(f'm_goalset {m_goalset.shape}')
                goalset_w_radius = torch.cat([m_goalset, radius*torch.ones((m_goalset.shape[0], 1), device=m_goalset.device)], dim=1)
                collision_flag_any = check_collisions(goalset_w_radius)
                # print(collision_flag_any)
                collision_flags.append(collision_flag_any)
                # if collision_flag_any:
                #     fig, axs = plt.subplots(1,1,figsize=(12,12))
                #     axs.scatter(goalset_w_radius[:,0].cpu().detach(), goalset_w_radius[:,1].cpu().detach())

                #     for xx, yy in goalset_w_radius[:,:2].cpu().detach():
                #         circle1 = plt.Circle((xx, yy), 1.0, color='g', alpha=0.4)
                #         axs.add_patch(circle1)
                #     plt.show()
        
        # print('-'*13, 'GT', '-'*13)
        for gt_goalset in ground_truth_goalsets:
            # for m_goalset in goalset:
            gt_goalset = gt_goalset.squeeze(1)
            gt_goalset_w_radius = torch.cat([gt_goalset, radius*torch.ones((gt_goalset.shape[0], 1), device=gt_goalset.device)], dim=1)
            # print(gt_goalset_w_radius.shape)
            collision_flag_any = check_collisions(gt_goalset_w_radius)
            gt_collision_flags.append(collision_flag_any)
            # print(collision_flag_any)
            # if collision_flag_any:
            #     fig, axs = plt.subplots(1,1,figsize=(12,12))
            #     axs.scatter(gt_goalset_w_radius[:,0].cpu().detach(), gt_goalset_w_radius[:,1].cpu().detach())

            #     for xx, yy in gt_goalset_w_radius[:,:2].cpu().detach():
            #         circle1 = plt.Circle((xx, yy), 1.0, color='b', alpha=0.4)
            #         axs.add_patch(circle1)
            #     plt.show()
        ...
        if num_batch>-1 and d_idx>=num_batch:
            break
    print(f'collision rate {sum(collision_flags)/len(collision_flags)}, radius={radius}')
    print(f'GT_collision rate {sum(gt_collision_flags)/len(gt_collision_flags)}, radius={radius}')
...
if __name__ == '__main__':
    ...
    # model_path = './models/MGA-M32_Joint-attn-minFDE2.93.ckpt' 
    model_path = './models/MGA-M32_Marginal-attn-minFDE2.62.ckpt'
    eval_device = 'cuda:0'
    model = torch.load(model_path, map_location=eval_device)
    model.eval()
    model.args['batch_size'] = 32
    model.args['device'] = eval_device
    model.args['dec_type'] = 'MGA' # 'mtp_goal_veh' # 'mtpGoal'
    model.args['num_val_batch'] = 500
    ...
    loader  = DataLoader(dataset, batch_size=model.args['batch_size'], shuffle=False, num_workers=2)
    #################################
