...
################################################################################
## 准备好 WOMD 的数据集
DATASET_FOLDER = '/home/users/ntu/baichuan/scratch/sim/Waymo_Dataset/testing'
...
################################################################################
from viz_rollouts_tar_change_TCL import find_WOMD_scenario
...
womd_scenario = find_WOMD_scenario(scenario_id, start_from=29600)
...
for d in loader:
    ...
    plt.savefig(f'./ego_change_TCL_{plotter.scenario_id}-{plotter.ego_id}.png', bbox_inches='tight')
    plt.close()
    ...
    plt.title(f'Scenario: {plotter.scenario_id} -- Ego: {plotter.ego_id}')
    plt.savefig(f'./ego_change_TCL_{plotter.scenario_id}-{plotter.ego_id}_changeTCL.png', bbox_inches='tight')
    plt.close()
