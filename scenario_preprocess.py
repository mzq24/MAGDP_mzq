...
from utils.map_utils import get_polylines, resample_centerline_nonShapely

class Scenario_Pre():
    def __init__(self, tf_record_data, for_sub='') -> None:
    ...
    def get_TCL_from_CCLs(self, agent_future, CCLs, radius=2, printtt=False):
        """ 如果 goal 和 CCL 距离超过 2m 就不能作为 TCL 
        CCLs 是一个 元组列表 [(ccl_path, resampled_ccl_line, ccl_attr, ccl_line)]
        resampled_ccl_line 可能太稀疏了，所以用数据中原始的 ccl_line 来计算距离 """
        # for ccl in CCLs:
        # assert agent_future.shape[0]>0 , f'agent_future {agent_future.shape}'
        if agent_future.shape[0]==0:
            return np.array([False for i in range(len(CCLs))])
        CCLs_distance_to_agent = [LineString(ccl[3][:,:2]).distance(Point(agent_future[-1, :2])) for ccl in CCLs]
        if printtt:
            print([ccl[3][:,:2].shape for ccl in CCLs])
            print(f'CCLs_distance_to_agent {CCLs_distance_to_agent}')
            print(f'agent_future {agent_future.shape}')
        ## 如果 goal 到所有 CCLs 的最小距离大于 radius，就返回全部 False
        if len(CCLs_distance_to_agent)<1:
            return np.array([])
        elif min(CCLs_distance_to_agent)>radius:
            return np.array([False for i in range(len(CCLs_distance_to_agent))])
        elif min(CCLs_distance_to_agent)<=radius:
            tcl_mask = [True if d==min(CCLs_distance_to_agent) else False for d in CCLs_distance_to_agent]
            ## 防止 tcl_mask 中有多个 True
            only_tcl_mask = np.array([index == tcl_mask.index(True) for index in range(len(tcl_mask))])
            return only_tcl_mask
    ...
    def get_hist(self, agn_id, sdc_centered=True):
        ...
        if self.for_sub == 'testing':
            assert len(agn_track)==11 
        else:
            assert len(agn_track)==91 
    ...
    def get_FAKE_future(self):
        return np.empty((80,6))
    ...
    def get_future(self, agn_id, sdc_centered=True):
        if self.for_sub == 'testing':
            return self.get_FAKE_future, False
        agn_track = self.cur_scenario.tracks[agn_id].states
        assert len(agn_track)==91 
        agn_fut, fut_valid = self.get_xyzvxvyh_from_track(agn_track[11:])
        goal_valid = agn_track[-1].valid
        ...
        if sdc_centered: # 如果要以 self driving car 为中心处理数据的话，需要转换到 SDC 坐标系下
            ...
            ## 坐标转换
            agn_fut = self.convert_raw_coords_to_sdc_frame(agn_fut, sdc_cur_xyzh[:2], sdc_cur_xyzh[2], sdc_cur_xyzh[3], state='xyzvxvyh')
            return agn_fut, np.all(fut_valid), goal_valid, cur_sdc_id, sdc_cur_xyzh
        else:
            return agn_fut, fut_valid
    ...
    def add_an_agent_to_a_graph(self, dg, agn_id, agn_type):
        ...
        agn_fut, fut_valid, goal_valid, _, _  = self.get_future(agn_id, sdc_centered=True)
        # print(f'agn_fut: {agn_fut.shape}')
        ...
        dg.add_node(agn_id, node_name=agn_node_name, node_feature=agn_hist.astype(np.float32), fut=agn_fut.astype(np.float32), fut_valid=fut_valid, goal_valid=goal_valid, node_type=agn_type, attr=agn_attr.astype(np.float32)) # Other agent node
        ...
        for nbr_id in agn_nbrs:
            if dg.has_node(nbr_id):
                ...
                ## 加上双向的边
                dg.add_edge(nbr_id, agn_id, edge_name=f'{nbr_id}->{agn_id}', edge_type=f'{nbr_type}-{agn_type}')
                dg.add_edge(agn_id, nbr_id, edge_name=f'{agn_id}->{nbr_id}', edge_type=f'{agn_type}-{nbr_type}') 
        return dg

    def add_CCLs_of_an_agent_to_a_graph(self, dg, agn_id, agn_type):
        agn_CCLs = self.get_CCLs_of_an_agent(agn_id, agn_type)
        agn_fut, agn_fut_valid  = self.get_future(agn_id, sdc_centered=False)
        agn_TCL_mask = self.get_TCL_from_CCLs(agn_fut[agn_fut_valid], agn_CCLs, radius=2)
        # print(f'agn_TCL_mask {agn_TCL_mask}')
        ccl_of_goal_valid_agn = nx.get_node_attributes(dg, 'goal_valid')[agn_id]
        ccl_of_fut_valid_agn  = nx.get_node_attributes(dg, 'fut_valid')[agn_id]
        ...
        for i, agn_ccl in enumerate(agn_CCLs):
            ...
            ccl_type = f'{agn_type[:3]}CCL' if not agn_TCL_mask[i] else f'{agn_type[:3]}TCL'
            ccl_id   = f'{agn_id}CCL{i}'    if not agn_TCL_mask[i] else f'{agn_id}TCL{i}'
            if self.for_sub:
                ccl_type = f'{agn_type[:3]}CCL'
                ccl_id   = f'{agn_id}CCL{i}'
            fake_fut = self.get_FAKE_future()
            # print(f'ccl_id {ccl_id}, ccl_type {ccl_type}')
            ccl_node_name = f'{self.cur_scenario.scenario_id}-{ccl_id}'
            dg.add_node(ccl_id, node_name=ccl_node_name, node_feature=local_ccl_6.astype(np.float32), fut=fake_fut.astype(np.float32), fut_valid=ccl_of_fut_valid_agn, goal_valid=ccl_of_goal_valid_agn, node_type=ccl_type, attr=ccl_attr.astype(np.float32)) # 
        ...
        if len(agn_CCLs)==0 or np.all(agn_TCL_mask == False):
            ## 找不到 Agent 的 CCL 或 没有合适的 TCL 的话，就构造一个 Fake TCL 便于处理计算
            fake_ccl_type = f'{agn_type[:3]}TCL_FAKE'
            fake_ccl_id   = f'{agn_id}TCL_FAKE'
            if self.for_sub:
            ...
            dg.add_node(fake_ccl_id, node_name=fake_ccl_node_name, node_feature=fake_hist.astype(np.float32), fut=fake_fut.astype(np.float32), fut_valid=ccl_of_fut_valid_agn, goal_valid=ccl_of_goal_valid_agn, node_type=fake_ccl_type, attr=fake_ccl_attr.astype(np.float32)) # 
